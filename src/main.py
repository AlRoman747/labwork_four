...
def main() -> None:
    ...
    def functions(book, removed_book):
        ...
        return {    "add": (book_list.add, book_dict.add, book),
                    ...
                    "search_by_isbn": (book_dict.get_by_isbn, random.choice(list(isbns))),
                    "search_by_year": (book_dict.get_by_year, random.choice(list(years))),
                    "search_by_author": (book_dict.get_by_author, random.choice(list(authors))),
    ...
    def get_commands(n):
        for i in range(1, n+1):
            ...
            if random_command == "add":
                ...
                print(func[0](rand_year))
            elif random_command == "search_by_author":
                rand_author = random.choice(list(authors))
                print(f"случайное событие №{i} - поиск книги по isbn: {rand_author}")
                print(func[0](rand_author))
